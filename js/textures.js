/* ───────────────────────────────────────────────
    TREE TEXTURES  (canvas-generated, pixel-art style)
    makeWoodTexture · makeLeafDiscTexture · makeLeafVerticalTexture
    + pre-built shared texture instances used by trees.js
─────────────────────────────────────────────── */

/**
 * Generates a tiled pixel-art wood texture on a canvas.
 * Horizontal planks with slight color variation, knots, and grain lines.
 */
    ...
    for (let row = 0; row < th / plankH; row++) {
        ...
        for (let py = row * plankH; py < (row + 1) * plankH; py++) {
            for (let px = 0; px < tw; px++) {
                // subtle grain noise
                const noise = (Math.sin(px * 0.7 + py * 1.3) * 0.5 + 0.5) * 12 - 6;
                const rr = Math.min(255, Math.max(0, r + noise));
                const gg = Math.min(255, Math.max(0, g + noise * 0.6));
                const bb = Math.min(255, Math.max(0, b + noise * 0.3));
...
/**
 * Generates a pixel-art leaf canopy disc texture with organic silhouettes,
...
function makeLeafDiscTexture(size = 128, layer = 0) {
    ...
    const cellSize = 6; // pixel-art cell size

    // 1. Draw organic pixel-art twigs radiating from center (underneath leaves)
    ...
    for (let y = 0; y < size; y += cellSize) {
        for (let x = 0; x < size; x += cellSize) {
            ...
            const gapNoise = Math.sin(x * 0.15) * Math.cos(y * 0.15) + Math.sin(x * 0.35 + y * 0.2);
    ...
    // 3. Scan and draw dark pixel-art outlines on the silhouette boundaries
    const imgData = ctx.getImageData(0, 0, size, size);
    const data = imgData.data;
    const outlinePixels = [];

    for (let y = cellSize; y < size - cellSize; y += cellSize) {
        for (let x = cellSize; x < size - cellSize; x += cellSize) {
            ...
            if (alpha > 0) {
                ...
                const leftAlpha   = data[(y * size + (x - cellSize)) * 4 + 3];
                const rightAlpha  = data[(y * size + (x + cellSize)) * 4 + 3];
                const topAlpha    = data[((y - cellSize) * size + x) * 4 + 3];
                const bottomAlpha = data[((y + cellSize) * size + x) * 4 + 3];
    ...
    // Draw the pixel boundaries in outline color
    ctx.fillStyle = '#143516';
    outlinePixels.forEach(p => {
...
/**
 * Generates a pixel-art vertical tree canopy texture with organic silhouette,
...
function makeLeafVerticalTexture(w = 128, h = 128) {
    ...
    for (let y = 0; y < h; y += cellSize) {
        ...
        for (let x = 0; x < w; x += cellSize) {
            ...
            const gapNoise = Math.sin(x * 0.15) * Math.cos(y * 0.15) + Math.sin(x * 0.35 + y * 0.25);
    ...
    for (let y = cellSize; y < h - cellSize; y += cellSize) {
        for (let x = cellSize; x < w - cellSize; x += cellSize) {
            ...
            if (alpha > 0) {
                const leftAlpha   = data[(y * w + (x - cellSize)) * 4 + 3];
                const rightAlpha  = data[(y * w + (x + cellSize)) * 4 + 3];
                const topAlpha    = data[((y - cellSize) * w + x) * 4 + 3];
                const bottomAlpha = data[((y + cellSize) * w + x) * 4 + 3];
