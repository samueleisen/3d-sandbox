...
function createHouse(px, pz, scale = 1.0) {
    ...
    // 1. Walls (Box)
    const wWidth  = 55 * scale;
    const wHeight = 42 * scale;
    const wDepth  = 55 * scale;
    ...
    const rRadius = 42 * scale; // corner radius to slightly overhang walls
    ...
    const dWidth  = 14 * scale;
    const dHeight = 22 * scale;
    const dDepth  = 2  * scale;
    ...
    doorMesh.position.set(0, dHeight / 2, wDepth / 2 + dDepth / 2 - 1.0 * scale); // slightly embedded
    doorMesh.castShadow = true;
    doorMesh.receiveShadow = true;
    houseGroup.add(doorMesh);

    // 4. Windows (Left and Right sides)
    const winSize  = 12 * scale;
    const winDepth = 2  * scale;
    ...
    winLeft.position.set(-wWidth / 2 - winDepth / 2 + 1.0 * scale, wHeight * 0.6, 0);
    ...
    winRight.position.set(wWidth / 2 + winDepth / 2 - 1.0 * scale, wHeight * 0.6, 0);
    ...
    chimneyMesh.position.set(-wWidth * 0.28, wHeight + cHeight / 2 - 4 * scale, -wDepth * 0.28);
